    ...
    <el-card class="box-card">
      <el-table :data="tableData" style="width: 100%" size="small" stripe fit highlight-current-row v-loading="tableLoding">
        ...
        <el-table-column fixed="right" label="操 作" width="120" header-align="center" align="center">
          <template slot-scope="scope">
            <!-- <el-button type="text" size="small" v-if="btnPower.checkBtn && scope.row.computationStatus == 'to_computation'" @click="currentId = scope.row.id; checkVisible = true">核算</el-button> -->
            <el-button type="text" size="small" v-if="btnPower.checkBtn && scope.row.computationStatus == 'to_manager_computation'" @click="currentId = scope.row.id;readonly = false; checkVisible = true">审 核</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      currentId: null,
      fixedParams: {computationStatusList:['to_computation','to_remit','remit', 'to_manager_computation', 'no_pass','over_due'], isManage: true},
      defaultParams: {computationStatus: 'to_manager_computation'},
      krShopList: [],
      btnPower: {
        checkBtn: true,
        detail: true,
