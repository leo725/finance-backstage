<template>
  <div class='menu-wrapper'>
    <template v-for="item in routes">
      <router-link v-if="!item.hidden&&item.noDropdown&&item.children" :to="item.path+'/'+item.children[0].path">
      ...
      <el-submenu v-if="!item.noDropdown&&!item.hidden" :index="item.name">
        ...
        <template v-if='!child.hidden' v-for="child in item.children">
          ...
          <router-link v-else :to="item.path+'/'+child.path">
            <el-menu-item :index="item.path+'/'+child.path" :data-index="item.path+'/'+child.path" @click="clearPageParams(item.path+'/'+child.path)">
...
<script>
import { pageParams } from 'utils/vx'
export default {
  name: 'SidebarItem',
  props: {
    routes: {
  ...
  methods:{
    clearPageParams(path){
      pageParams.remove(path)
    }
  }
}
</script>